#!/usr/bin/env bash
# D-10: the realigned learner on `Assoc` under the repaired multiplier
# instrument (PREREGISTRATION D-10, history 14.21). Same shape as the D-9
# driver, whose smoke gate stopped on a railed latency multiplier; that file
# stays as the D-9 record.
#
...
[[ -x "$PY" ]] || PY=python3
LEARN=results/learner-assoc-d10
SMOKE=results/learner-smoke-d10
mkdir -p gate1

banner() { printf '\n==============================================================\n%s\n==============================================================\n' "$1"; }

banner "PHASE 0/4  manifests: add the telemetry-unit latency references (D-10)"
# Deterministic over the existing calibration artifacts: the guard limits come
# out byte-identical and six *_latency_avg_ns_telemetry_* fields are added.
# The manifests are gitignored data, so the node regenerates its own copies.
for T in 2 6 10; do
  "$PIPE/06_calibrate_live_guard.py" \
    --selection-manifest "baseline_selection/assoc-v1/10M/T$T/baseline_slo.json" \
    --calibration-results results/guard/assoc-v1/calibration \
    --repeats 3 --output "baseline_slo/assoc-v1/10M/T$T/baseline_slo.json"
done
...
RESULTS_ROOT="$SMOKE" \
DB_ROOT=.dbbench_pipeline_dbs/learner-smoke-d10 \
...
if w_reward is None or abs(w_reward / w_eval - 1.0) > 0.01:
    raise SystemExit(f"D-10 prediction 1 FAILS on the smoke arm: reward W {w_reward} vs evaluator W {w_eval:.3f}")
print(f"D-10 prediction 1 (smoke): reward W {w_reward:.4f} vs evaluator W {w_eval:.4f} -> within 1%")
if max(lam.values()) >= lam_max:
    raise SystemExit(f"a multiplier railed at LAMBDA_MAX on the smoke arm: {lam}")
if lam["latency"] > 5.0:
    raise SystemExit(f"D-10 prediction 2 FAILS on the smoke arm: lambda_latency {lam['latency']:.2f} > 5")
...
if early:
    raise SystemExit(f"D-10 prediction 3 FAILS on the smoke arm: {early} deep releases below phi 0.95; the mask did not reach the plant")
print("D-10 prediction 3 (smoke): zero deep releases below phi 0.95")
PY
...
BASELINE_SLO_DIR=baseline_slo \
RESULTS_ROOT="$LEARN" \
DB_ROOT=.dbbench_pipeline_dbs/learner-assoc-d10 \
RESUME=1 CONFIRM_EXPERIMENTS=YES \
"$PIPE/03_run_experiments.sh"
...
"$PIPE/17_analyze_shadow_overrides.py" \
  ...
  --manifest-root baseline_slo --output gate1/d10_conditional.json \
  > gate1/d10_conditional.txt 2>&1 || rc=$?
echo "  stage 17 exit=$rc (per-arm errors only; see gate1/d10_conditional.txt)"

banner "PHASE 4/4  score D-10 predictions"
"$PY" - <<'PY' | tee gate1/d10_verdict.txt
...
import os
LEARN = os.environ.get("D10_LEARN_ROOT", "results/learner-assoc-d10")
...
print("\nD-10: the realigned learner on Assoc, repaired multiplier instrument")
print("=" * 100)
rows = {}; per_arm = []
for T in (2, 6, 10):
...
print("\nD-10 predictions")
p1 = all(i["w_reward"] is not None and abs(i["w_reward"] / i["w_eval"] - 1) <= 0.01 for i in per_arm)
print(f"1  reward W == evaluator W within 1%, every arm:                      {V(p1)}")
...
print("8  E-5 above 1% in >= 2 cells (D-8 p5): see gate1/d10_conditional.txt")
...
echo "  gate1/d10_verdict.txt                         D-10 predictions 1-7, 9"
echo "  gate1/d10_conditional.json/.txt               E-5 (D-10 prediction 8)"
