...
def _training_loop(agent: DQNAgent, stop_event: threading.Event) -> None:
    """Background thread: drains the replay buffer continuously."""
    while not stop_event.is_set():
        agent.train_step()
        # Small sleep avoids spinning when buffer is below MIN_REPLAY_SIZE.
    ...
    try:
        while True:
            ...
            while "\n" in buf:
                ...
                print(
                    f"[server] recv  f0={msg['f0']:.3f} df0={msg['df0']:.3f}"
                    f" s0={msg['s0']:.3f} pcb={msg['pcb']:.3f}"
                    f" reward={reward:.4f}"
                )

                # Inference only — response is sent before any training.
                action = agent.observe(state, reward, done)

                response = json.dumps({"action": action}) + "\n"
                conn.sendall(response.encode("utf-8"))

                print(f"[server] sent  step={agent.step} action={_ACTION_NAMES.get(action, action)}")

                # In single-threaded mode train inline after the response is sent.
...
def run_server(socket_path: str, agent: DQNAgent, tracker: MetricsTracker, io_log) -> None:
    ...
    print(f"[server] training mode: {'async (background thread)' if config.ASYNC_TRAINING else 'inline (after response)'}")
    ...
    if config.ASYNC_TRAINING:
        train_thread = threading.Thread(
            target=_training_loop, args=(agent, stop_train), daemon=True, name="rl-trainer"
        )
        train_thread.start()
